(function(){
  ...
  const panel = $("#panel"), bg = $("#bg"), game = $("#game"), ctx = game.getContext("2d");
  (function loop(ts){
  ...
  function hedge(r){ ctx.fillStyle='#2a8d4e'; ctx.fillRect(r.x,r.y,r.w,r.h); ctx.strokeStyle='#0d411e'; ctx.lineWidth=2; ctx.strokeRect(r.x,r.y,r.w,r.h); }
  function glade(g){ ctx.fillStyle='#b5ec7f'; ctx.fillRect(g.x,g.y,g.w,g.h); ctx.strokeStyle='#2b5a2a'; ctx.lineWidth=2; ctx.strokeRect(g.x,g.y,g.w,g.h); }
  function firefly(t,ts){ const cx=t.x+t.w/2, cy=t.y+t.h/2, r=6+2*(.5+.5*Math.sin(ts*0.01+(cx+cy)*.02)); const grd=ctx.createRadialGradient(cx,cy,0,cx,cy,r*3); grd.addColorStop(0,'rgba(255,220,0,.9)'); grd.addColorStop(1,'rgba(255,220,0,0)'); ctx.fillStyle=grd; ctx.beginPath(); ctx.arc(cx,cy,r*3,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#ffdc00'; ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2); ctx.fill(); }
  function tx(s,x,y,sz=18,c='#f3ffe9'){ ctx.font=`bold ${sz}px ui-rounded, ui-sans-serif`; ctx.lineWidth=4; ctx.strokeStyle='rgba(0,0,0,.6)'; ctx.strokeText(s,x,y); ctx.fillStyle=c; ctx.fillText(s,x,y); }
  let keys={};
  function boot(i){ hide(); runLevel(Math.max(0, Math.min(i, LV.length-1))); }
  function runLevel(i){
    const L=LV[i]; let t=0, tokens=L.tok.map(o=>({ ...o }));
    ...
    function step(dt){
      ...
      for(const e of enemies){ const tgt=e.path[e.idx+e.dir]||e.path[e.idx]; let dx=tgt.x-(e.r.x+e.r.w/2), dy=tgt.y-(e.r.y+e.r.h/2); const dist=Math.hypot(dx,dy)||1; const s=e.speed*1.25; if(dist<=s){ e.r.x=tgt.x-e.r.w/2; e.r.y=tgt.y-e.r.h/2; if(e.bounce){ if(e.idx+e.dir>=e.path.length-1 || e.idx+e.dir<0) e.dir*=-1; else e.idx+=e.dir; } else { e.idx=(e.idx+1)%e.path.length; } } else { e.r.x+=Math.round(s*dx/dist); e.r.y+=Math.round(s*dy/dist); } if(cRect(p.x,p.y,p.r,e.r)){ die(); return; } }
    ...
    function draw(ts){
      ctx.clearRect(0,0,960,520); for(const w of walls) hedge(w); glade(goal); for(const to of tokens) firefly(to,ts);
      for(const e of enemies){ ctx.fillStyle='#e14d4d'; ctx.fillRect(e.r.x,e.r.y,e.r.w,e.r.h); ctx.strokeStyle='#5a1010'; ctx.lineWidth=2; ctx.strokeRect(e.r.x,e.r.y,e.r.w,e.r.h); }
      ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill(); ctx.lineWidth=2; ctx.strokeStyle='#8dcf7a'; ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.stroke();
